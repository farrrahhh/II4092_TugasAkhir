...
\texttt{exercises} &
\texttt{exercise\_id, topic\_id, question, difficulty\_level, generated\_by\_llm} &
...
\begin{table}[H]
    ...
    \label{tab:api-auth}
    \begin{tabular}{|p{1.5cm}|p{3.8cm}|p{3.3cm}|p{4cm}|}
    \hline
    \textbf{Metode} & \textbf{\textit{Endpoint}} & \textbf{\textit{Body} / \textit{Header}} & \textbf{Fungsi} \\
    \hline
    POST & \texttt{/api/auth/register} & \texttt{name, email, password} & Mendaftarkan pengguna baru dan mengembalikan JWT \textit{token} \\
    \hline
    ...
    \hline
    PUT & \texttt{/api/auth/profile} & \texttt{name, email,} \texttt{current\_password,} \texttt{new\_password} & Memperbarui nama, \textit{email}, atau kata sandi pengguna \\
    \hline
    \end{tabular}
\end{table}
...
\begin{table}[H]
    ...
    \hline
    POST & \texttt{/api/latihan/} \texttt{\{topic\_slug\}/generate} & \texttt{jumlah, kelemahan,} \texttt{tipe\_paksa,} \texttt{topik\_referensi} & Menghasilkan soal latihan adaptif berbasis LLM sesuai topik dan kelemahan pengguna \\
    \hline
    POST & \texttt{/api/latihan/} \texttt{\{topic\_slug\}/evaluasi} & \texttt{soal, jawaban} & Mengevaluasi satu jawaban latihan dan mengembalikan \textit{feedback} \\
    \hline
